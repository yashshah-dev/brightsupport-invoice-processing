...
/**
 * Generate PDF and return as bytes (for further processing like adding form fields)
 */
export async function generatePDFBytes(invoiceData: InvoiceData): Promise<Uint8Array> {
  const doc = new jsPDF();
  const pageWidth = doc.internal.pageSize.width;
  ...
  try {
    ...
    if (resp.ok) {
      ...
      doc.addImage(dataUrl, 'PNG', pageWidth / 2 - 30, 10, 60, 20);
    }
  } catch (error) {
  ...
  doc.text(formatCurrency(invoiceData.total), pageWidth - 20, finalY, { align: 'right' });
